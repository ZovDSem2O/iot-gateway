...
static uint8 zclSampleSw_ProcessInReportCmd( zclIncomingMsg_t *pInMsg )
{
  ...
  osal_memset(resstr,0,10);     //½â¾ö×Ö·û´®ÀïÃæ¶àÓàÂÒÂëÎÊÌâ
  ...
  for ( i = 0; i < reportCmd->numAttr; i++ )
  {
    ...
    {
      //////   TODO Ñ§ÉúÍê³É ½«reportCmd->attrList[i].attrData ¸´ÖÆµ½resstr ²¢·¢ËÍ/////
      strncpy(resstr,(char const *)reportCmd->attrList[i].attrData,6);        //¿½±´6¸öÊý¾Ý ±ÈÈç14 70
      HalUARTWrite(0, "T&H:", 4);       //ÌáÊ¾½ÓÊÕµ½Êý¾Ý
...
//ÏÂÃæÕâ¸öº¯ÊýÊÇÖÕ¶Ë·¢ËÍÊý¾Ýµ½Ð­µ÷Æ÷º¯Êý
  ...
  static void zclSampleSw_ReportTest(void)
  {
      ...
      zclReportCmd_t *reportCmd;
      uint8 i, temp[3], humidity[3], strTemp[7]; //¶¨ÒåÎÂ¶È×Ö·û´® Êª¶È×Ö·û´® ×îºóÒ»¸öÊÇÎÂ¶ÈºÍÊª¶ÈÒ»ÆðµÄ×Ö·û´®
  
      //////   TODO Ñ§ÉúÍê³É µã²¥Ð­µ÷Æ÷µÄµØÖ·¶¨Òå µã²¥ÐÎÊ½Ö±½Ó´«µÝ¸øÐ­µ÷Æ÷0x0000 /////
      //////   TODO Ñ§ÉúÍê³É ²É¼¯ DHT11 ÎÂÊª¶ÈÊý¾Ý ÎÂÊª¶ÈÊý¾ÝÊ®Î»ºÍ¸öÎ»          /////
      //////   ´æÔÚ wendu_shi wendu_ge(ÎÂ¶È)  shidu_shi shidu_ge£¨Êª¶È£©         /////
      ...
      Delay_ms(500);
      //////                        END                                         /////
      //////////// TODO Ñ§ÉúÍê³É ½«ÎÂÊª¶ÈµÄ×ª»»³É×Ö·û´®ÒÑ±¸ÓÃ·¢ËÍ////////////////////
      //////////// TODO Ñ§ÉúÍê³É ²¢´æÔÚ temp[3], humidity[3]     ////////////////////
      ...
      humidity[2] = '\0';
      //////////// TODO Ñ§ÉúÍê³É´æÔÚ strTemp ×Ö·û´®ÀïÃæ ÒÔ¡°T&H: XX XX¡±(Ò»¸ö¿Õ¸ñ)////
      osal_memcpy(strTemp, temp, 2); 
      osal_memcpy(&strTemp[2], "  ", 2);
      osal_memcpy(&strTemp[4], humidity, 3); //×îºóÒ»¸ö½áÊø·ûÒ²¿½±´¹ýÈ¥
      //////                        END                                        /////
      ...
      reportCmd->attrList[0].attrData = (uint8 *)osal_mem_alloc(sizeof(strTemp));
      ...
      strcpy((char *)reportCmd->attrList[0].attrData,(char const *)strTemp);  //×Ö·û´®Êý¾Ý
